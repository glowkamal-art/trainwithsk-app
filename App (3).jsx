...
function addByDuration(startDate, value, unit) {
  return unit === "days" ? addDays(startDate, Number(value)) : addMonths(startDate, Number(value));
}
...
function toRow(c, category) {
  return {
    ...
    duration_unit: c.durationUnit || "months",
...
function fromRow(r) {
  return {
    ...
    durationUnit: r.duration_unit || "months",
...
function Dashboard({ onLogout }) {
  ...
  async function saveClient(data) {
    const endDate = addDays(addByDuration(data.startDate, data.durationMonths, data.durationUnit), Number(data.bonusDays || 0));
    ...
    const setForCategory = category === "regular" ? setClients : setPtClients;
    setForCategory((prev) => {
      const exists = prev.some((c) => c.id === toSave.id);
      return exists ? prev.map((c) => (c.id === toSave.id ? toSave : c)) : [...prev, toSave];
    });
    ...
    if (modal.mode === "add" && data.referredBy && data.referredBy.trim()) {
      ...
      if (matches && matches.length > 0) {
        ...
        const newEnd = addDays(addByDuration(ref.start_date, ref.duration_months, ref.duration_unit), newBonus);
        await supabase.from("clients").update({ bonus_days: newBonus, end_date: newEnd }).eq("id", ref.id);
        const refUpdate = (arr) => arr.map((c) => (c.id === ref.id ? { ...c, bonusDays: newBonus, endDate: newEnd } : c));
        setClients(refUpdate);
        setPtClients(refUpdate);
      }
    }
    setModal(null);
  }
  ...
  async function addPayment(client, amount, mode, date) {
    ...
    const apply = (arr) => arr.map((c) => (c.id === client.id ? { ...c, payments: updated } : c));
    setClients(apply);
    setPtClients(apply);
  ...
  async function addProgress(client, weight, date) {
    ...
    const apply = (arr) => arr.map((c) => (c.id === client.id ? { ...c, progress: updated } : c));
    setClients(apply);
    setPtClients(apply);
...
function ClientModal({ isPT, mode, data, onClose, onSave }) {
  ...
  async function handlePhotoUpload(e) {
    ...
    try {
      ...
      const path = `${form.id}-${Date.now()}.jpg`;
    ...
    <Modal title={(mode === "add" ? "Add " : "Edit ") + (isPT ? "PT client" : "client")} onClose={onClose} width={560}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div style={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <label style={{ ...btnOutline, display: "inline-block", opacity: uploading ? 0.6 : 1 }}>
        ...
        <Field label="Duration">
          <div style={{ display: "flex", gap: 6 }}>
            <input type="number" min="1" style={{ ...inputStyle, flex: 1 }} value={form.durationMonths} onChange={(e) => set("durationMonths", e.target.value)} />
            <select style={{ ...inputStyle, width: 110 }} value={form.durationUnit} onChange={(e) => set("durationUnit", e.target.value)}>
